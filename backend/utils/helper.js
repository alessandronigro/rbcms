...
async function getMailFormat(format = "mailformat") {
    try {
        const connFormat = await getConnection("IFAD", "wpacquisti"); // ✅ stesso DB del VB
...
function logwrite(message, folder = "logs") {
    try {
...
/**
 * ✅ Recupera mail BCC da convenzione WP + email fattura da forma4
 */
async function getBCC(iduser) {
    try {
        const connForma = await getConnection("IFAD", "forma4");
        const connWP = await getConnection("IFAD", "wpacquisti");
...
function resolvePlatformFromHost(host = "", piattaforma = "") {
    ...
    if (piattaforma.includes("forma4")) return { connKey: "IFAD", db: "forma4" };
...
async function getLastTest(idcourse, idst, firstname, lastname, db, savefile = false, options = 1, res = null) {
    const conn = await getConnection("IFAD", "forma4"); // ✅ sempre questa connessione
    ...
    try {
        // Estrai ultimo iduser da idst (es. "12,13,15")
        const ids = idst.split(",");
        const lastid = ids[ids.length - 1];

        // 1️⃣ Recupera ultimo test
        ...
        const [rows] = await conn.query(`
            ...
            WHERE idreference IN (
                ...
                WHERE idcourse = ? AND ${whereClause}
            )
            AND iduser = ?
            ORDER BY date_attempt DESC
            LIMIT 1
        `, [idcourse, idcourse, options, lastid]);
    ...
    `;
      ...
      `, [q.idquest]);
        ...
        const filename = `Test Finale - ${code} - ${nomeutente} - ${idst}.pdf`;
        ...
        if (savefile) {
            ...
            console.log("📤 Invio file report:", filename);
            stream.pipe(res);
            return;
        }
    ...
    try {
     ...
     WHERE a.idcourse=? AND a.iduser=? LIMIT 1`,
        ...
        if (db === "forma4") {
            [oggettididattici] = await conn.query(`
                SELECT DISTINCT a.title, '' AS modulo, a.title, b.dateAttempt, b.first_complete, b.last_complete,
                       b.status, b.idreference, b.objecttype, b.firstattempt
                FROM learning_organization a
        ...
        for (const row of oggettididattici) {
            ...
            let dataCompleto = dayjs(row.first_complete) || dayjs(row.dateAttempt) || "";
            ...
            if (row.objecttype === "test") {
                ...
                const [testRows] = await conn.query(
                    "SELECT score FROM learning_testtrack WHERE iduser=? AND idtest=? LIMIT 1",
                    [iduser, row.idreference]
                );
                voto = testRows?.[0]?.score || "";
            }
        ...
        console.log("Connessioni trovate:", connRows.length);
...
async function getNomeCorsoById(idcourse, db = "forma4") {
    // se db è un oggetto, prendi la chiave o il nome
    const dbName = typeof db === "object" ? db?.database || db?.name || "forma4" : String(db);
    const conn = await getConnection("IFAD", dbName);
