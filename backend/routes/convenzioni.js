...
function normalizeWebField(dbName) {
    if (dbName === process.env.MYSQL_FORMA4) return "newindirizzoweb";
...
/***************************************
* ✅ FULL — SOLO corsi di tblprezzi (IFAD process.env.MYSQL_FORMA4), storico per varianti
***************************************/
router.get("/:codice/full", async (req, res) => {
    ...
    try {
        // Connessioni
        const connWP = await getConnection("wpacquisti");       // 2011→2018
        const connIFAD = await getConnection(process.env.MYSQL_FORMA4);             // 2025→oggi
        const connEFAD = await getConnection("newformazionein");    // 2018→2024
        const connSITE = await getConnection("formazionein");       // 2011→2018
    ...
    `;
        ...
        // 6) Nomi corsi (per canonical) presi da IFAD (process.env.MYSQL_FORMA4) → code = canonical
        ...
        for (const row of prezziRows) {
            ...
            // Nome: da IFAD process.env.MYSQL_FORMA4 se disponibile, altrimenti usa il canonical
