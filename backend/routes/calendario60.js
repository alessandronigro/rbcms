 ...
 * → Iscrive l'utente al test corretto su process.env.MYSQL_FORMA4
 */
...
router.post("/sessione/:idsessione/sblocca-test", async (req, res) => {
    ...
    try {
        ...
        // ✅ VB: update process.env.MYSQL_FORMA4.learning_Testtrack set checktest=0 where iduser=.. and idtest in (969,1372)
        await connForma.query(
...
async function insertPrenotazione60h(idsessione, iduser, idcourse) {
    ...
    // Anagrafica → sincronizza da process.env.MYSQL_FORMA4
...
async function insertSessione260h({
    ...
    try {
        ...
        // 4️⃣ flagevent=0 su process.env.MYSQL_FORMA4
        await connForma.query(
            `UPDATE learning_certificate_assign 
             SET flagevent = 0
             WHERE id_user = ? AND id_course = ?`,
