...
export default function ModalePrenotatiAmm({ idSessione, onClose, onReloadCalendar }: ModalePrenotatiAmmProps) {
    const [loading, setLoading] = useState(true);
    const [sessione, setSessione] = useState<any>(null);

    const fetchData = async () => {
        setLoading(true);
        const res = await fetch(`/api/finecorsoamm/sessione/${idSessione}/dettaglio`);
    ...
    const deleteSessione = async () => {
        ...
        await fetch(`/api/finecorsoamm/sessione/${sessione.id}`, { method: "DELETE" });
        ...
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
            ...
            <div className="bg-white rounded-lg shadow-lg w-[700px] p-5 relative">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl font-bold">📌   Gestione Sessione {new Date(sessione.dataesame).toLocaleString("it-IT", {
                        day: "2-digit",
                        month: "2-digit",
                        year: "numeric",
                        hour: "2-digit",
                ...
                {loading ? (
                    ...
                    <>
                        ...
                        <div className="bg-gray-50 p-3 rounded-md text-sm mb-4">
                            <p><b>Nome:</b> {sessione.nome_utente} {sessione.cognome_utente}</p>
                            <p><b>Email:</b> {sessione.email_utente}</p>
                            <p><b>CF:</b> {sessione.cf_utente}</p>
                            <p><b>Telefono:</b> {sessione.telefono_utente}</p>
                            <p><b>Indirizzo:</b> {sessione.indirizzo_utente}</p>
                        </div>
