...
export default function FattureList({ which }: { which: Which }) {
  ...
  const markRead = async (id: number) => {
    ...
    setRows((prev) =>
      prev.map((r) => (r.id === id ? { ...r, readfatt: 1 } : r)),
    );
  };
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="flex flex-wrap items-end gap-3">
        ...
        <button
          onClick={exportZip}
          className="px-3 py-1 bg-green-600 text-white rounded h-9"
        >
          Esporta ZIP mese
        </button>
        ...
        <div className="ml-auto text-sm font-medium text-gray-600">
          {loading
            ? "Caricamento..."
            : `${meta?.total ?? rows.length} righe — Tot: ${totale.toLocaleString(
              "it-IT",
              {
                style: "currency",
                currency: "EUR",
        ...
        <table className="min-w-[1100px] w-full text-sm">
          ...
          <tbody>
            {rows.map((r) => (
              <tr
                key={r.id}
                className={`border-t ${r.readfatt ? "text-gray-700" : "font-semibold"}`}
              >
                <td className="p-2">
                  {r.readfatt ? (
                ...
                <td className="p-2 text-right">
                  {r.imponibile?.toLocaleString("it-IT")}
                </td>
                <td className="p-2 text-right">
                  {r.IVA?.toLocaleString("it-IT")}
                </td>
                <td className="p-2 text-right">
                  {r.importofattura?.toLocaleString("it-IT")}
                </td>
