...
export default function IscrizioniAca() {
  ...
  const fetchOrdini = async (p = 1) => {
    ...
    try {
      const res = await fetch(
        `/api/iscrizioni/nova?page=${p}&limit=${limit}&db=novastudia`
      );
  ...
  const interrompi = async (orderId: string) => {
    ...
    await fetch(
      `/api/iscrizioni/ordini/${encodeURIComponent(orderId)}?db=novastudia`,
      {
        method: "PATCH",
  ...
  const iscrivisito = async (idordine: number, nuovo: boolean) => {
    ...
    const res = await fetch(`/api/iscrizioni/weborders?db=novastudia`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    if (!json.success) {
      await showAlert(json.error || "Errore iscrizione");
      return;
    }

    await showAlert("Iscrizione completata");
    fetchOrdini(page);
  };
  ...
  const iscrivisitosingolo = async (
    ...
    const j = await res.json();
    if (!j.success) {
      await showAlert(j.error || "Errore iscrizione singolo");
      return;
    }

    await showAlert("Iscrizione singolo completata");
    fetchOrdini(page);
  };
