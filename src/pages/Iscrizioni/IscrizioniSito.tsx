...
interface Corsista {
    ...
    esitoEmail?: { to: "ok" | "ko" | "-"; bcc: "ok" | "ko" | "-"; pec: "ok" | "ko" | "-" };
}
...
export default function IscrizioniSito() {
    ...
    const fetchCorsisti = async (orderId: string) => {
        ...
        try {
            const res = await fetch(`/api/iscrizioni/ordini/${encodeURIComponent(orderId)}/corsisti`);
    ...
    const reinvia = async (orderId: string) => {
        ...
        const res = await fetch(`/api/iscrizioni/ordini/${encodeURIComponent(orderId)}/reinvia`, { method: "POST" });
    ...
    const segnala = async (orderId: string) => {
        ...
        const res = await fetch(`/api/iscrizioni/ordini/${encodeURIComponent(orderId)}/segnala`, { method: "POST" });
        const j = await res.json();
    ...
    // Modifica corsista
    const EditCorsistasito = (orderId: string, c: Corsista) => setEditCorsista({ orderId, corsista: c });

    // Totale pagina
    const totale = useMemo(() => rows.reduce((s, r) => s + (r.fatturato || 0), 0), [rows]);

    const badge = (val?: "ok" | "ko" | "-") => {
        const color = val === "ok" ? "bg-green-100 text-green-700" : val === "ko" ? "bg-red-100 text-red-700" : "bg-gray-100 text-gray-600";
        return <span className={`px-1.5 py-0.5 rounded text-[11px] font-semibold ${color}`}>{val || "-"}</span>;
    };

    return (
        <div className="p-4 sm:p-6 space-y-4">
            <h1 className="text-lg sm:text-xl font-semibold">📦 Iscrizioni da sito</h1>
            ...
            {loading ? (
                ...
                <div className="overflow-x-auto border rounded-md shadow-sm">
                                ...
                                return (
                                    <React.Fragment key={rowKey}>
                                        <tr className={`border-t hover:bg-gray-50 ${row.order_status === "completed" ? "bg-orange-50" : ""}`}>
                                            <td className="text-center cursor-pointer select-none" onClick={() => toggleRow(rowKey, orderKey)}>
                                                {isOpen ? "▼" : "▶"}
                                            </td>
                                            ...
                                            <td className="p-2">
                                                <div className="flex flex-wrap gap-1">
                                                    ...
                                                    <span className="px-2 py-1 text-xs bg-gray-200 rounded">{row.order_id}</span>
                                                </div>
                                            </td>
                                                                    ...
                                                                    {corsisti.map((c, idx) => {
                                                                        ...
                                                                        return (
                                                                            <tr key={corsistaKey} className="border-t hover:bg-gray-50">
                                                                                ...
                                                                                <td className="p-2">{c.codice_corso} - {c.corso_title}</td>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-center text-sm p-3 border-t bg-gray-50 gap-2">
                        <span>
                            Pagina {page} / {Math.ceil(total / limit)} ({total.toLocaleString("it-IT")} ordini)
                        </span>
            ...
            {!loading && (
                <div className="pt-4 text-right text-sm text-gray-700 font-semibold">
                    Totale pagina: {totale.toLocaleString("it-IT", { style: "currency", currency: "EUR" })}
                </div>
            )}
            ...
            {editOrdine && (
                <ModalEditOrdine
                    ...
                    onSaved={() => { setEditOrdine(null); fetchOrdini(page); }}
                />
            )}
            {editCorsista && (
                <ModalEditCorsista
