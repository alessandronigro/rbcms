...
export default function ModalePrenotati({ idSessione, onClose, onReloadCalendar }: ModalePrenotatiProps) {
    const [loading, setLoading] = useState(true);
    const [sessione, setSessione] = useState<any>(null);

    const fetchData = async () => {
        setLoading(true);
        const res = await fetch(`/api/finecorso60h/sessione/${idSessione}/dettaglio`);
    ...
    const deleteSessione = async () => {
        ...
        await fetch(`/api/finecorso60h/sessione/${sessione.id}`, { method: "DELETE" });
