        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-xl w-[95%] max-w-5xl p-6 relative">
                ...
                <p className="text-gray-700 text-sm mb-4">
                    <b>{row.lastname} {row.firstname}</b> • {row.code} — {row.convenzione}
                </p>
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label className="text-sm text-gray-700">Seconda data proposta</label>
                        <input
                            type="datetime-local"
...
export default function FineCorso60h() {
    ...
    const loadData = async () => {
        setLoading(true);
        const res = await fetch("/api/finecorso60h");
    ...
    const handleSaveModal = async (form: any) => {
        ...
        await fetch("/api/finecorso60h/email/proposta", {
            method: "POST",
    ...
    return (
        <div className="p-6">
            ...
            <div className="mb-4 text-xs text-gray-600 space-y-1">
                <div><span className="inline-block w-3 h-3 bg-green-200 mr-2 rounded"></span>Da confermare</div>
                <div><span className="inline-block w-3 h-3 bg-green-600 mr-2 rounded"></span>Confermata</div>
                <div><span className="inline-block w-3 h-3 bg-red-400 mr-2 rounded"></span>Buon fine NO</div>
                <div><span className="inline-block w-3 h-3 bg-pink-300 mr-2 rounded"></span>Buon fine SI</div>
                <div><span className="inline-block w-3 h-3 bg-orange-500 mr-2 rounded"></span>🔔 Sessioni già create</div>
            </div>

            <h1 className="text-xl font-bold mb-4">🧾 Fine Corso 60 ore — Gestione Sessioni</h1>
            ...
            {loading ? (
                ...
                <table className="min-w-full bg-white rounded shadow text-xs">
                    <thead className="bg-gray-100 text-gray-700 uppercase text-[11px]">
                        <tr>
                            <th className="p-2 cursor-pointer w-[200px]" onClick={() => sortTable("lastname")}>COGNOME</th>
                            <th className="p-2 cursor-pointer w-[120px]" onClick={() => sortTable("firstname")}>NOME</th>
                            <th className="p-2 w-[110px]">CORSO</th>
                            <th className="p-2 cursor-pointer w-[170px]" onClick={() => sortTable("on_date")}>
                                DATA DOC FINE CORSO
                            </th>
                            <th className="p-2 w-[170px]">CONVENZIONE</th>
                            <th className="p-2 cursor-pointer w-[140px]" onClick={() => sortTable("date_inscr")}>
                                DATA ISCRIZIONE
                            </th>
