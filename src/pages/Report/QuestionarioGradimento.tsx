...
export default function ReportQuestionariA() {
  ...
  useEffect(() => {
    fetch("/api/iscrizioni/convenzioni")
      .then((r) => r.json())
      .then((list) => {
        const mapped = (list || []).map((x: any) => ({
          id: x.value ?? x.id ?? x.name,
          nome: x.nome ?? x.name ?? x.label,
        }));
  ...
  const toggleAttiva = async (row: Row, current: 0 | 1) => {
    try {
      ...
      setRows((prev) =>
        prev.map((r) =>
          r.id_track === row.id_track ? { ...r, attiva: current ? 0 : 1 } : r,
        ),
      );
    } catch (e) {
          ...
          <table className="min-w-[1100px] w-full text-sm text-gray-800">
            ...
            <tbody>
              ...
              {(rows as FreeRow[]).map((r) => (
                <tr
                  key={r.id_track}
                  className="hidden sm:table-row border-t hover:bg-gray-50"
                >
                  {/* Data */}
                  <td className="p-2 text-xs">{fmtDT(r.date_attempt)}</td>
                  ...
                  <td className="p-2 text-center">
                    <span
                      className={`px-2 py-[2px] text-[11px] rounded font-semibold ${r.attiva
                  ...
                  <td colSpan={5} className="p-2">
                    <div className="bg-white border rounded shadow px-3 py-2 space-y-2">
                      ...
                      <div className="flex justify-between items-center">
                        <span
                          className={`px-2 py-[2px] text-[11px] rounded font-bold ${r.attiva
                            ? "bg-green-600 text-white"
                            : "bg-gray-400 text-white"
                            }`}
                        >
