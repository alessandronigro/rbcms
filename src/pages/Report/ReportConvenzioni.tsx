...
function StatusBadge({ stato, percent }: { stato?: string; percent: number }) {
    ...
    return <span className={`px-2 py-[2px] rounded text-xs font-semibold ${cls}`}>{txt}</span>;
}
...
export default function ReportConvenzione() {
    ...
    const [from, setFrom] = useState<string>(fromDefault.toISOString().slice(0, 10));
    ...
    const exportCSV = () => {
        const header = ["Cognome", "Nome", "Email", "CF", "Stato", "%", "Data iscr.", "Data compl."];
            ...
            <div className="p-4 space-y-4">
                ...
                <div className="flex flex-col sm:flex-row sm:items-end gap-3">
                    ...
                    <div className="sm:ml-auto">
                        ...
                        <input type="text" value={q} onChange={(e) => setQ(e.target.value)} className="border rounded px-2 py-1" />
                    </div>
                </div>
                            ...
                            {!loading &&
                                filtered.map((r) => (
                                    <tr key={r.id} className="border-t hover:bg-gray-50">
                                        ...
                                        <td className="p-2"><StatusBadge stato={r.stato} percent={r.percent} /></td>
