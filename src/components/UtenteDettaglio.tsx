...
export default function UtenteDettaglio({ detail, onAction }: Props) {
    ...
    const [attUrlMap, setAttUrlMap] = useState<{ [key: number]: string | null }>({});
    const [errorMap, setErrorMap] = useState<{ [key: number]: string | null }>({});
    const [loading, setLoading] = useState(false);
    if (!detail) return <p className="text-gray-500">Nessun dettaglio disponibile</p>;
    console.log("📌 DETAIL OBJECT:", detail);

    const { user, courses = [], fields = [] } = detail;

    const getField = (label: string) =>
        fields.find(f => f.translation?.toLowerCase() === label.toLowerCase())?.user_entry || "";
    ...
    const handleGeneraAttestato = async (idcorso: string) => {
        ...
        try {
            ...
            if (scelta) {
                ...
                if (!data.success) throw new Error(data.error || "Errore invio attestato");
                ...
                const data = await res.json();
                if (!data.success) throw new Error(data.error || "Errore generazione attestato");
                url = data.file;
        ...
        <div className="bg-gradient-to-b from-gray-50 to-gray-100 min-h-screen p-6">
            ...
            <div className="bg-white border rounded-lg p-4 shadow-sm mb-4">
                ...
                <div className="grid grid-cols-2 gap-x-10 gap-y-2 text-sm text-gray-800 mt-4">
                    <p><b>Username:</b> {cleanUser}</p>
                    <p><b>Password:</b> {password}</p>

                    {getField("cellulare") && (
                        <p><b>Cellulare:</b> {getField("cellulare")}</p>
                    )}

                    <p><b>Email utente:</b> {getField("Indirizzo Email")?.toLowerCase() || user.email}</p>

                    {getField("PEC") && (
                        <p><b>PEC:</b> {getField("PEC")}</p>
                    )}

                    <p><b>Codice Fiscale:</b> {getField("Codice Fiscale") || user.cf}</p>

                    {getField("Ragione Sociale") && (
                        <p><b>Ragione Sociale:</b> {getField("Ragione Sociale")}</p>
                    )}

                    {getField("Sede") && (
                        <p><b>Sede:</b> {getField("Sede")}</p>
                    )}

                    {getField("P.I./C.F.") && (
                        <p><b>P.I./C.F.:</b> {getField("P.I./C.F.").toUpperCase()}</p>
                    )}

                    {getField("PEC Fattura") && (
                        <p><b>PEC Fattura:</b> {getField("PEC Fattura")}</p>
                    )}

                    {getField("Codice Destinatario") && (
                        <p><b>Codice Destinatario:</b> {getField("Codice Destinatario")}</p>
                    )}

                    <p><b>Convenzione:</b> {getField("Convenzione") || user.convenzione}</p>
                </div>
            </div>

            {/* ✅ CORSI */}
            {courses.map((c) => (
                <div key={c.idCourse} className="bg-white border rounded-lg p-4 shadow-sm mb-4">
                    ...
                    <div className="text-sm space-y-1 text-gray-700">
                        ...
                        <p><b>Ultimo Accesso al corso:</b> {c.last_access_course || "—"}</p>
                        <p><b>Ultimo Accesso piattaforma:</b> {detail.user.lastenter || "—"}</p>

                        <p>
                            <b>Stato:</b> {c.stato_descrizione || c.status}
                            {c.evaso ? " /Evaso" : ""} —
                            <b>Data Scadenza:</b> {c.date_expire_validity || "N/A"}
                        </p>
                        ...
                        <p><b>Invio Attestato:</b> {c.date_invio || "—"}</p>
                        <p><b>Ore VideoCorsi:</b> {c.ore_video || "00m 00s"}</p>
                        <p><b>Ore Totali:</b> {c.ore_totali || "00h 00m 00s"}</p>
                    </div>
                    {/* 🔘 Azioni */}
                    <div className="flex flex-wrap gap-2 mb-3">
                        <button
                            onClick={() => onAction?.("unlock", c)}
                        ...
                        <button
                            ...
                            disabled={loading}
                            className={`inline-flex items-center gap-1 px-3 py-1 rounded text-sm text-white ${loading
                                ? "bg-gray-400"
